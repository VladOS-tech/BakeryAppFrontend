...
interface RequestFormProps {
  ...
  loading?: boolean;
  submitLabel?: string;
  onSubmit: (items: { product_id: number; quantity: number }[]) => void;
}
...
export default function RequestForm({
  ...
  const [selected, setSelected] = useState<{ [id: number]: boolean }>(() => {
    ...
    products.forEach(p => { if (initialItems[p.id]) obj[p.id] = true; });
    return obj;
  });
  ...
  const toggleSelect = (id: number) => {
    setSelected(sel => {
      ...
      if (copy[id]) {
        delete copy[id];
        // очищаем и количество
        setQuantities(q => { const q2 = { ...q }; delete q2[id]; return q2; });
      } else {
        copy[id] = true;
        ...
        renderItem={({ item }) => (
          <View style={styles.productRow}>
            <TouchableOpacity onPress={() => toggleSelect(item.id)} style={styles.checkbox}>
              {selected[item.id]
                ? <MaterialIcons name="check-box" size={24} color="#42a5f5" />
